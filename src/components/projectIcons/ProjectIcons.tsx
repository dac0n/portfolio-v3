    ...
    <div className={twMerge("flex h-full w-full flex-row", className)}>
      {projects.map((project) => (
        <div
          key={project}
          className={twMerge(
            `"inline-block transition-colors duration-300 ${isActive?.(project) ? "border-projectIconOutline-active" : "border-projectIconOutline-inactive hover:border-projectIconOutline-hovered"} ${!!onClick && "desktop:p-2 cursor-pointer rounded-xl border-2 p-1"} "cursor-pointer"`,
            iconClassName,
          )}
